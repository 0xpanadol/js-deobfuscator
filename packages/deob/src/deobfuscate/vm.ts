...
export function createBrowserSandbox(): Sandbox {
  return async (code: string) => {
    // Use indirect eval (globalThis.eval) for browser/worker environments
    // This evaluates in the global scope, similar to isolated-vm but without isolation
    const indirectEval = globalThis.eval
    return indirectEval(code) as unknown
  }
}
